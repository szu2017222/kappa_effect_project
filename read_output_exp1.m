function read_output_exp1(velocity,subjectID)
% filenames
filename_pro = ['Kappa3Tone1_vel',num2str(velocity),'_',num2str(subjectID),'_protocol.txt'];
filename_log = ['Kappa3Tone1_vel',num2str(velocity),'_',num2str(subjectID),'_log.txt'];
% read files
...
pitch_cond_num = 5;
pitch_conds = [-2 -1 0 1 2];
pitch_cond_strs = ["pitch_neg2","pitch_neg1","pitch_0","pitch_pos1","pitch_pos2"];
shortlong_button = 1;
longshort_button = 2;
...
output_data_block.conditions = repmat({table_template},block_num,1);
output_data_block.condition_correct = repmat({table_template},block_num,1);
output_data_block.condition_longshort = repmat({table_template},block_num,1);
...
case pitch_conds(3)
output_data_block.conditions{block_iter,1}.pitch_0(time_cond_strs(time_conds==time_iter)) = find(output_data_block.timeoffset{block_iter,1} == time_iter & output_data_block.pitchoffset{block_iter,1} == pitch_iter);
output_data_block.condition_correct{block_iter,1}.pitch_0(time_cond_strs(time_conds==time_iter)) = ismember(output_data_block.conditions{block_iter,1}.pitch_0(time_cond_strs(time_conds==time_iter)),output_data_block.correct{block_iter,1});
output_data_block.condition_longshort{block_iter,1}.pitch_0(time_cond_strs(time_conds==time_iter)) = ismember(output_data_block.conditions{block_iter,1}.pitch_0(time_cond_strs(time_conds==time_iter)),output_data_block.longshort{block_iter,1});
case pitch_conds(4)
